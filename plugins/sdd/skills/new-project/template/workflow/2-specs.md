...
1. `# NNN — <MOD> · <title>`, where `<MOD>` is the spec's short module code (`CFG`), unique across specs.
2. `- [ ] Spec approved   <- only the user marks this`
3. `## Goal` — one sentence.
4. `## Scope` — what it covers and, if anything, what it leaves **out of scope**, with the reason.
5. `## Requirements` — `FR-<MOD>-<n>`, each a checkable case: condition → observable result, including rejections and boundaries. Every row carries its priority and its T/A/I/D/U class.
6. `## Non-functional requirements` (`NFR-<n>`, with class) and `## Constraints` (`C<n>`, with origin), when the spec has any.
7. `## Reference docs` — the sections of `docs/*.md` it rests on.
...
- **Mandatory** is V1; **Desirable** may be left out without invalidating the delivery.
- A figure not yet calibrated is named as such and given no value; it is listed in `architecture.md` *Open decisions*.
