...
No step invents a figure: one still uncalibrated (`architecture.md` *Decisiones abiertas*) is read from config and fails actionably if missing.
...
Mark a step `[x]` only when its case goes green. If implementation proves the plan wrong, stop and change the plan with the user; every step comes from the plan.
    ...
    Spec: [spec.md](spec.md). Verificación: <método> (`docs/verification.md`).
