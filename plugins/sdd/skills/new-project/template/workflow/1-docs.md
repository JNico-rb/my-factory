...
3. Closing a decision = moving it from `architecture.md` *Decisiones abiertas* to *Decisiones cerradas*, with its reason and the alternatives discarded written in the section that owns it. Reopening a closed decision needs a stated reason in the same commit.
...
4. A new verification method goes to `verification.md` with its row in the coverage tables; an accepted risk goes to *Riesgos aceptados (U)* with name and motive.
